...
// Serial interfaces
HardwareSerial sim7600(1); // UART1 for modem
HardwareSerial rs485(2);   // UART2 for Modbus
...
const unsigned char SERVER_PUBLIC_KEY[] = {     // Uncompressed point indicator
    0x04, 0x71, 0x52, 0xAA, 0x83, 0xD8, 0x45, 0xE8, 0x5C, 0x0A, 0xE8, 0xE0, 0x84, 0x3C, 0x3D, 0x65, 0x7A,
...
String lastAckMsg = "";
unsigned long lastChunkNum = 0xFFFFFFFF;
...
void postTransmission() {
    ...
    delay(100); // Allow inverter time to respond
}
...
void writeSingleRegister(uint16_t reg, uint16_t value) {
    ...
    if (result == node.ku8MBSuccess) {
        response = "MODBUS:WRITE_REG:SUCCESS:REG:" + String(reg) + ":VALUE:" + String(value, HEX);
    } else {
        response = "MODBUS:WRITE_REG:ERROR:0x" + String(result, HEX);
    }
    publishMQTT(mqtt_topic_send, response.c_str());
}
...
void writeSingleCoil(uint16_t reg, bool state) {
    ...
    String response;
    if (result == node.ku8MBSuccess) {
        response = "MODBUS:WRITE_COIL:SUCCESS:REG:" + String(reg) + ":STATE:" + String(state);
    } else {
        response = "MODBUS:WRITE_COIL:ERROR:0x" + String(result, HEX);
    }
    publishMQTT(mqtt_topic_send, response.c_str());
}
...
void writeMultipleRegisters(uint16_t reg, uint16_t* values, uint8_t count) {
    ...
    String response;
    if (result == node.ku8MBSuccess) {
        response = "MODBUS:WRITE_MULTI:SUCCESS:REG:" + String(reg) + ":COUNT:" + String(count);
        for (uint8_t i = 0; i < count; i++) {
            response += ":VALUE" + String(i) + ":" + String(values[i], HEX);
        }
    } else {
        response = "MODBUS:WRITE_MULTI:ERROR:0x" + String(result, HEX);
    }
    publishMQTT(mqtt_topic_send, response.c_str());
}
...
void readHoldingRegisters(uint16_t startReg, uint8_t count) {
    ...
    String response;
    if (result == node.ku8MBSuccess) {
        response = "MODBUS:READ:SUCCESS:REG:" + String(startReg) + ":COUNT:" + String(count);
        for (uint8_t i = 0; i < count; i++) {
            response += ":VALUE" + String(i) + ":" + String(node.getResponseBuffer(i), HEX);
        }
    } else {
        response = "MODBUS:READ:ERROR:0x" + String(result, HEX);
    }
    publishMQTT(mqtt_topic_send, response.c_str());
}
...
// Existing functions (unchanged unless modified for Modbus)
String base64_encode(const unsigned char *input, size_t len) {
    String output = "";
    int i = 0, j = 0;
    ...
    while (i < len) {
        ...
        if (j == 3 || i == len) {
            ...
            char_array_4[1] = ((char_array_3[0] & 0x03) << 4) + ((j > 1 ? (char_array_3[1] & 0xf0) : 0) >> 4);
...
void handleMessage(String topic, String payload) {
    ...
    if (topic == PROVISION_RESPONSE_TOPIC && waitingForProvisionResponse) {
        if (payload.startsWith("CREDENTIALS:")) {
            ...
            String device_id_new = creds.substring(deviceIdStart, usernameStart);
            ...
            saveCredentials(device_id_new, username, password);
        ...
        } else if (payload.startsWith("MODBUS:")) {
            if (!isProvisioned) {
                SerialMon.println("Device not provisioned, ignoring Modbus command");
                return;
            }
            // Parse Modbus command
            if (payload.startsWith("MODBUS:READ:DEVICE:" + device_id + ":")) {
                String params = payload.substring(payload.indexOf(":", 14) + 1);
                int colon = params.indexOf(':');
                if (colon != -1) {
                    uint16_t reg = parseNumber(params.substring(0, colon));
                    uint8_t count = parseNumber(params.substring(colon + 1));
                    readHoldingRegisters(reg, count);
                }
            } else if (payload.startsWith("MODBUS:WRITE_REG:DEVICE:" + device_id + ":")) {
                String params = payload.substring(payload.indexOf(":", 22) + 1);
                int colon = params.indexOf(':');
                if (colon != -1) {
                    uint16_t reg = parseNumber(params.substring(0, colon));
                    uint16_t value = parseNumber(params.substring(colon + 1));
                    writeSingleRegister(reg, value);
                }
            } else if (payload.startsWith("MODBUS:WRITE_COIL:DEVICE:" + device_id + ":")) {
                String params = payload.substring(payload.indexOf(":", 23) + 1);
                int colon = params.indexOf(':');
                if (colon != -1) {
                    uint16_t reg = parseNumber(params.substring(0, colon));
                    bool state = parseNumber(params.substring(colon + 1)) == 1;
                    writeSingleCoil(reg, state);
                }
            } else if (payload.startsWith("MODBUS:WRITE_MULTI:DEVICE:" + device_id + ":")) {
                String params = payload.substring(payload.indexOf(":", 24) + 1);
                int colon1 = params.indexOf(':');
                if (colon1 != -1) {
                    uint16_t reg = parseNumber(params.substring(0, colon1));
                    String rest = params.substring(colon1 + 1);
                    int colon2 = rest.indexOf(':');
                    if (colon2 != -1) {
                        uint8_t count = parseNumber(rest.substring(0, colon2));
                        if (count > 10) {
                            publishMQTT(mqtt_topic_send, "MODBUS:WRITE_MULTI:ERROR:Count too large");
                            return;
                        }
                        uint16_t values[10];
                        String valueStr = rest.substring(colon2 + 1);
                        int lastColon = -1;
                        for (uint8_t i = 0; i < count; i++) {
                            int nextColon = valueStr.indexOf(':', lastColon + 1);
                            String val = (nextColon == -1) ? valueStr.substring(lastColon + 1) : valueStr.substring(lastColon + 1, nextColon);
                            values[i] = parseNumber(val);
                            lastColon = nextColon;
                            if (nextColon == -1 && i < count - 1) {
                                publishMQTT(mqtt_topic_send, "MODBUS:WRITE_MULTI:ERROR:Not enough values");
                                return;
                            }
                        }
...
bool uploadCertificate() {
    ...
    if (response.indexOf(String("+CCERTLIST: \"") + cert_name + "\"") >= 0) {
        SerialMon.println("Certificate exists");
        return true;
    }
    ...
    if (!mqttStatus.clientAcquired) {
        ...
        if (modem.waitResponse(5000L, response) != 1) {
            ...
            if (response.indexOf("+CMQTTACCQ: 0,19") >= 0) {
                // Client index 0 already acquired, attempting to release...    
                ...
                if (modem.waitResponse(5000L) != 1) {
                    ...
                    SerialAT.println(accqCmd);
                    if (modem.waitResponse(5000L, response) != 1 || response.indexOf("+CMQTTACCQ: 0,0") < 0) return false;
                } else {
                    SerialAT.println(accqCmd);
                    if (modem.waitResponse(5000L, response) != 1 || response.indexOf("+CMQTTACCQ: 0,0") < 0) return false;
                }
...
bool subscribeMQTT(const char *topic) {
    ...
    if (modem.waitResponse(2000L, response) != 1 || response.indexOf("OK") < 0) return false;
...
bool publishMQTT(const char *topic, const char *message) {
    ...
    if (modem.waitResponse(500L, ">") != 1) return false;
    SerialAT.print(topic);
    if (modem.waitResponse(500L) != 1) return false;
...
bool stopMQTT() {
        ...
        while (millis() - startTime < 15000L && !(gotOK && gotURC)) {
            if (SerialAT.available()) {
                ...
                if (line.length() > 0) {
                    discResponse += line + "\n";
                    if (line.indexOf("OK") >= 0) gotOK = true;
                    if (line.indexOf("+CMQTTDISC: 0,0") >= 0) gotURC = true;
                }
            } else {
                delay(100);
            }
    ...
    if (mqttStatus.clientAcquired) {
        ...
        if (modem.waitResponse(5000L, relResponse) != 1 || relResponse.indexOf("OK") < 0) {
            SerialMon.println("Warning: Failed to release MQTT client");
        }
...
void processOTAFirmware(const String &topic, byte *payload, unsigned int dataLen) {
    ...
    if (payloadStr.startsWith("OTA:BEGIN:")) {
        ...
        if (deviceColon != -1) {
            String deviceId = payloadStr.substring(deviceColon + 8);
        ...
        if (deviceColon != -1) {
            otaSignature = payloadStr.substring(nextColon + 1, deviceColon);
        } else {
            otaSignature = payloadStr.substring(nextColon + 1);
        }
...
void saveCredentials(String device_id, String username, String password) {
    ...
    preferences.putBytes("user_iv", user_iv, 16);

    unsigned char enc_pass[32], dec_pass[32] = {0};
    unsigned char pass_iv[16];
    ...
    preferences.putBytes("pass_iv", pass_iv, 16);
...
bool requestCredentialsFromServer() {
    ...
    if (hasPendingNonce && waitingForProvisionResponse) {
        ...
        if (mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &entropy, NULL, 0) != 0) {
            ...
            if (ret != 0 || olen != 65 || pubkey_buf[0] != 0x04) {
                regenerationSuccess = false;
            } else {
