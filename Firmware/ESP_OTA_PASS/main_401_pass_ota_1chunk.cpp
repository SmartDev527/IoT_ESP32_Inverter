...
String getIMEI()
{
    ...
    if (modem.waitResponse(2000L, response) == 1) {
        response.trim();
        // IMEI is typically 15 digits, extract only the number
        // Response format is usually just the IMEI number followed by OK
        int imeiStart = response.indexOf('\n') + 1;
        int imeiEnd = response.indexOf('\n', imeiStart);
        if (imeiEnd == -1) imeiEnd = response.length();
        
        imei = response.substring(imeiStart, imeiEnd);
        imei.trim();
        
        // Verify it's a valid IMEI (15 digits)
        if (imei.length() == 15 && imei.toInt() > 0) {
            SerialMon.println("IMEI retrieved: " + imei);
        } else {
            SerialMon.println("Invalid IMEI format: " + imei);
            imei = "";
        }
    } else {
        SerialMon.println("Failed to get IMEI");
    }
