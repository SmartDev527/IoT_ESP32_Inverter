...
unsigned long provisionTimeout = 120000;
        ...
        if (tryStep("Setting up MQTT", setupMQTT()))
        {
            if (!isProvisioned)
            {
                if (requestCredentialsFromServer())
                {
                    ...
                    provisionStartTime = millis(); /// added
                }
                else
                {
...
void handleMessage(String topic, String payload)
{
    ...
    else if (topic == PROVISION_RESPONSE_TOPIC && !isProvisioned && waitingForProvisionResponse)
    {
        ...
        {
            String newPassword = decrypted.substring(9, decrypted.indexOf(":NONCE:"));
            String returnedNonce = decrypted.substring(decrypted.indexOf(":NONCE:") + 7);
            unsigned char storedNonce[16];
            preferences.begin("device-creds", true);
            preferences.getBytes("nonce", storedNonce, 16);
            preferences.end();
            String expectedNonce = base64_encode(storedNonce, 16);
            ...
            if (returnedNonce == expectedNonce) {
                ...
                SerialMon.println("Nonce mismatch in provisioning response");
                nextState(STATE_WAIT_PROVISION);
            }
            saveCredentials(newPassword);
...
bool requestCredentialsFromServer()
{
    ...
    String requestMsg = "IMEI:" + imei + ":NONCE:" + nonceStr;
