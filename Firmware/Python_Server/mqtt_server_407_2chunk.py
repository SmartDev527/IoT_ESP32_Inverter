import paho.mqtt.client as mqtt
import base64
import os
import json
from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
from cryptography.hazmat.primitives import hashes, serialization
from cryptography.hazmat.primitives.asymmetric import ec
from cryptography.hazmat.backends import default_backend
from hashlib import sha256
import threading
import time
import queue
import sys
import platform
import secrets
import logging

if platform.system() == "Windows":
    import msvcrt
else:
    import select

# MQTT Broker settings
BROKER = "u008dd8e.ala.dedicated.aws.emqxcloud.com"
PORT = 8883
USERNAME = "ESP32"
PASSWORD = "12345"
CA_CERT = "C:/Users/Milad/Documents/OTA_Python_Server/emqx_ca.crt"  # Update path
PROVISION_REQUEST_TOPIC = "dev_pass_req"
PROVISION_RESPONSE_TOPIC = "dev_pass_res"
OTA_TOPIC = "OTA_Update"
STATUS_TOPIC = "esp32_status"
COMMAND_TOPIC = "server_cmd"  # New topic for reset commands
CHUNK_SIZE = 1024

# Set up logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)
provisioning_lock = threading.Lock()
ota_file_path = None  # To store OTA file path globally if needed

# Load or generate static server key pair
KEY_FILE = "server_private_key.pem"
if os.path.exists(KEY_FILE):
    with open(KEY_FILE, "rb") as f:
        PRIVATE_KEY = serialization.load_pem_private_key(
            f.read(), password=None, backend=default_backend())
else:
    PRIVATE_KEY = ec.generate_private_key(ec.SECP256R1(), default_backend())
    pem = PRIVATE_KEY.private_bytes(
        encoding=serialization.Encoding.PEM,
        format=serialization.PrivateFormat.PKCS8,
        encryption_algorithm=serialization.NoEncryption()
    )
    with open(KEY_FILE, "wb") as f:
        f.write(pem)

PUBLIC_KEY = PRIVATE_KEY.public_key()
PUBLIC_KEY_PEM = PUBLIC_KEY.public_bytes(
    encoding=serialization.Encoding.PEM,
    format=serialization.PublicFormat.SubjectPublicKeyInfo
).decode('utf-8')

# Global variables
client = None
command_queue = queue.Queue()
chunk_ack_event = threading.Event()
last_chunk_sent = -1
chunk_buffer = {}
ota_in_progress = False
acknowledged_chunks = set()

def load_devices():
    try:
        with open("devices.json", "r") as f:
            return json.load(f)
    except FileNotFoundError:
        return {}

def save_devices(devices):
    with open("devices.json", "w") as f:
        json.dump(devices, f, indent=4)

def pad(data, block_size=16):
    padding_len = block_size - (len(data) % block_size)
    padding = bytes([padding_len] * padding_len)
    return data + padding

def encrypt_message(message, key, iv):
    cipher = Cipher(algorithms.AES(key), modes.CBC(iv), backend=default_backend())
    encryptor = cipher.encryptor()
    padded_message = pad(message.encode('utf-8'))
    encrypted = encryptor.update(padded_message) + encryptor.finalize()
    return base64.b64encode(encrypted).decode('utf-8')

def handle_provisioning(uuid, nonce, pubkey_b64):
    """Handle provisioning requests in a separate function"""
    with provisioning_lock:  # Ensure only one provisioning at a time
        devices = load_devices()
        logger.info(f"Processing provisioning for IMEI: {uuid}")
        
        try:
            # Generate or retrieve device key
            if uuid in devices:
                device_key = base64.b64decode(devices[uuid]["key"])
            else:
                device_key = secrets.token_bytes(32)  # 256-bit key
                devices[uuid] = {"key": base64.b64encode(device_key).decode('utf-8')}
                save_devices(devices)

            # Get password with timeout to prevent hanging
            print(f"Provide password for {uuid} (min 12 chars): ", end='', flush=True)
            password = None
            for _ in range(30):  # 30-second timeout
                if platform.system() == "Windows":
                    if msvcrt.kbhit():
                        password = input().strip()
                        break
                else:
                    if select.select([sys.stdin], [], [], 1.0)[0]:
                        password = sys.stdin.readline().strip()
                        break
                time.sleep(1)
            
            if not password:
                logger.error(f"Timeout waiting for password for {uuid}")
                return

            while len(password) < 12:
                logger.warning("Password must be at least 12 characters")
                print(f"Provide password for {uuid} (min 12 chars): ", end='', flush=True)
                password = input().strip()

            # Encrypt and send response
            iv = os.urandom(16)
            encrypted_pass = encrypt_message(password, device_key, iv)
            response = {
                "uuid": uuid,
                "nonce": nonce,
                "encrypted_pass": encrypted_pass,
                "iv": base64.b64encode(iv).decode('utf-8'),
                "server_pubkey": PUBLIC_KEY_PEM
            }
            client.publish(PROVISION_RESPONSE_TOPIC, json.dumps(response), qos=1)
            logger.info(f"Sent provisioning response for {uuid}")
        except Exception as e:
            logger.error(f"Provisioning failed for {uuid}: {e}")


def compute_firmware_hash(file_path):
    sha = sha256()
    with open(file_path, "rb") as f:
        while chunk := f.read(CHUNK_SIZE):
            sha.update(chunk)
    return sha.hexdigest()

def sign_firmware(file_path):
    with open(file_path, "rb") as f:
        data = f.read()
    digest = hashes.Hash(hashes.SHA256(), backend=default_backend())
    digest.update(data)
    hash_value = digest.finalize()
    signature = PRIVATE_KEY.sign(hash_value, ec.ECDSA(hashes.SHA256()))
    from cryptography.hazmat.primitives.asymmetric.utils import decode_dss_signature
    r, s = decode_dss_signature(signature)
    raw_signature = r.to_bytes(32, byteorder='big') + s.to_bytes(32, byteorder='big')
    return base64.b64encode(raw_signature).decode('utf-8')

def on_connect(client, userdata, flags, rc, properties=None):
    if rc == 0:
        logger.info("Connected to MQTT broker")
        client.subscribe(PROVISION_REQUEST_TOPIC, qos=1)
        client.subscribe(STATUS_TOPIC, qos=1)
        logger.info(f"Subscribed to {PROVISION_REQUEST_TOPIC} and {STATUS_TOPIC}")
    else:
        logger.warning(f"Connection failed with code {rc}, attempting reconnect")
        time.sleep(5)
        client.reconnect()

def send_ota_firmware(file_path):
    global last_chunk_sent, chunk_buffer, ota_in_progress, acknowledged_chunks
    MAX_RETRIES = 3
    RETRY_DELAY = 2
    FINAL_CONFIRMATION_TIMEOUT = 20

    try:
        if not os.path.exists(file_path):
            logger.error(f"File '{file_path}' not found")
            return

        file_size = os.path.getsize(file_path)
        total_chunks = (file_size + CHUNK_SIZE - 1) // CHUNK_SIZE
        logger.info(f"Starting OTA: File size: {file_size} bytes, Total chunks: {total_chunks}")

        with open(file_path, "rb") as f:
            magic_byte = f.read(1)
            if magic_byte != b'\xe9':
                logger.error("Invalid firmware file (magic byte != 0xE9)")
                return

            firmware_hash = compute_firmware_hash(file_path)
            signature = sign_firmware(file_path)
            begin_command = f"OTA:BEGIN:{file_size}:{firmware_hash}:{signature}"

            for attempt in range(MAX_RETRIES):
                client.publish(OTA_TOPIC, begin_command, qos=1)
                logger.info(f"Sent OTA:BEGIN (attempt {attempt + 1}/{MAX_RETRIES})")
                chunk_ack_event.clear()
                ota_in_progress = True
                if chunk_ack_event.wait(timeout=10):
                    logger.info("OTA:STARTED received")
                    break
                elif attempt == MAX_RETRIES - 1:
                    logger.error("Max retries reached for OTA:BEGIN")
                    ota_in_progress = False
                    return
                time.sleep(RETRY_DELAY)

            f.seek(0)
            chunk_num = 0
            total_bytes_sent = 0
            chunk_buffer.clear()
            acknowledged_chunks.clear()

            while chunk_num < total_chunks and ota_in_progress:
                remaining_bytes = file_size - total_bytes_sent
                if remaining_bytes < 0:
                    logger.error(f"total_bytes_sent ({total_bytes_sent}) exceeds file_size ({file_size})")
                    break
                
                chunk_size = min(CHUNK_SIZE, remaining_bytes)
                chunk = f.read(chunk_size)
                if len(chunk) == 0:
                    logger.error(f"No data read for chunk {chunk_num} at {total_bytes_sent} bytes")
                    break

                chunk_data = chunk_num.to_bytes(4, byteorder='big') + chunk
                encoded_chunk = base64.b64encode(chunk_data).decode('utf-8')
                chunk_buffer[chunk_num] = encoded_chunk

                for attempt in range(MAX_RETRIES):
                    client.publish(OTA_TOPIC, encoded_chunk, qos=1)
                    logger.info(f"Sent chunk {chunk_num} ({len(chunk)} bytes, attempt {attempt + 1}/{MAX_RETRIES})")
                    total_bytes_sent += len(chunk)
                    last_chunk_sent = chunk_num
                    chunk_ack_event.clear()
                    if chunk_ack_event.wait(timeout=20):
                        progress_percentage = (total_bytes_sent / file_size) * 100
                        logger.info(f"Chunk {chunk_num} acknowledged - OTA Progress: {progress_percentage:.2f}% ({total_bytes_sent}/{file_size} bytes)")
                        break
                    elif attempt == MAX_RETRIES - 1:
                        logger.error(f"Max retries reached for chunk {chunk_num}")
                        client.publish(OTA_TOPIC, "OTA:CANCELLED", qos=1)
                        ota_in_progress = False
                        return
                    logger.warning(f"Retrying chunk {chunk_num} after {RETRY_DELAY} seconds")
                    time.sleep(RETRY_DELAY)

                chunk_num += 1

            if chunk_num != total_chunks:
                logger.error(f"Sent {chunk_num} chunks, expected {total_chunks}")
                ota_in_progress = False
                return

            logger.info(f"All {total_chunks} chunks sent, awaiting final confirmation")
            while ota_in_progress and len(acknowledged_chunks) < total_chunks:
                chunk_ack_event.clear()
                progress_percentage = (total_bytes_sent / file_size) * 100
                logger.info(f"Acknowledged chunks: {len(acknowledged_chunks)}/{total_chunks} - OTA Progress: {progress_percentage:.2f}%")
                if not chunk_ack_event.wait(timeout=FINAL_CONFIRMATION_TIMEOUT):
                    missing_chunks = [i for i in range(total_chunks) if i not in acknowledged_chunks]
                    if missing_chunks:
                        missing_str = ",".join(map(str, missing_chunks))
                        client.publish(OTA_TOPIC, f"OTA:REQUEST:{missing_str}", qos=1)
                        logger.info(f"Requested missing chunks: {missing_str}")
                else:
                    logger.info("Received confirmation update")

            if ota_in_progress:
                client.publish(OTA_TOPIC, "OTA:END", qos=1)
                progress_percentage = (total_bytes_sent / file_size) * 100
                logger.info(f"OTA completed successfully - Final Progress: {progress_percentage:.2f}%")
    except Exception as e:
        logger.exception(f"Unexpected error during OTA: {e}")
        ota_in_progress = False
    finally:
        if not ota_in_progress:
            logger.info("OTA process terminated")

def on_message(client, userdata, msg):
    global last_chunk_sent, ota_in_progress, chunk_ack_event, acknowledged_chunks
    payload = msg.payload.decode('utf-8')
    topic = msg.topic
    logger.info(f"Received - Topic: {topic}, Payload: {payload[:50]}...")

    if topic == PROVISION_REQUEST_TOPIC:
        if payload.startswith("UUID:") and ":NONCE:" in payload and ":PUBKEY:" in payload:
            parts = payload.split(":NONCE:")
            uuid = parts[0][5:]
            rest = parts[1].split(":PUBKEY:")
            nonce = rest[0]
            pubkey_b64 = rest[1]
            # Process provisioning in a separate thread
            threading.Thread(target=handle_provisioning, args=(uuid, nonce, pubkey_b64), daemon=True).start()    
    elif topic == STATUS_TOPIC:
        if payload.startswith("OTA:PROGRESS:"):
            parts = payload.split(":")
            if len(parts) >= 6 and parts[4] == "DEVICE":
                chunk_num = int(parts[2])
                sizes = parts[3].split('/')
                received_size = int(sizes[0])
                total_size = int(sizes[1])
                device_id = parts[5]
                acknowledged_chunks.add(chunk_num)
                progress_percentage = (received_size / total_size) * 100
                logger.info(f"Progress for {device_id}: {received_size}/{total_size} bytes, chunk: {chunk_num} - {progress_percentage:.2f}%")
                if ota_in_progress and chunk_num == last_chunk_sent:
                    chunk_ack_event.set()
        elif payload == "OTA:STARTED":
            logger.info("ESP32 confirmed OTA start")
            if ota_in_progress:
                chunk_ack_event.set()
        elif payload.startswith("OTA:REQUEST:"):
            missing_chunks = payload.split(":")[2].split(",")
            for chunk_num in missing_chunks:
                chunk_num = int(chunk_num)
                if chunk_num in chunk_buffer:
                    client.publish(OTA_TOPIC, chunk_buffer[chunk_num], qos=1)
                    logger.info(f"Resent chunk {chunk_num}")
                    last_chunk_sent = chunk_num
                    chunk_ack_event.clear()
                    if not chunk_ack_event.wait(timeout=10):
                        logger.warning(f"Timeout waiting for chunk {chunk_num} ack")
        elif payload.startswith("OTA:ERROR:"):
            logger.error(f"OTA error from ESP32: {payload}")
            ota_in_progress = False
            chunk_ack_event.set()
        elif payload == "OTA:SUCCESS:PENDING_VALIDATION":
            logger.info("OTA completed on ESP32, pending validation")
            ota_in_progress = False
            chunk_ack_event.set()
        elif payload == "OTA:CANCELLED":
            logger.info("OTA cancelled by ESP32")
            ota_in_progress = False
            chunk_ack_event.set()

def input_handler():
    logger.info("Server running... Enter 'cmd' for commands.")
    input_buffer = ""

    while True:
        if platform.system() == "Windows":
            if msvcrt.kbhit():
                char = msvcrt.getch().decode('utf-8', errors='ignore')
                if char in ['\r', '\n']:
                    action = input_buffer.strip().lower()
                    if action == "cmd":
                        try:
                            command = input("Enter command ('OTA' or 'reverse old firmware'): ").strip().lower()
                            if command == "ota":
                                file_path = input("Enter firmware file path: ").strip()
                                if os.path.exists(file_path):
                                    send_ota_firmware(file_path)
                                else:
                                    logger.error(f"File not found: {file_path}")
                            elif command == "reverse old firmware":
                                client.publish(OTA_TOPIC, "reverse old firmware", qos=1)
                                logger.info("Sent: reverse old firmware")
                            else:
                                logger.warning("Invalid command")
                        except EOFError:
                            logger.error("Input interrupted (EOF). Please try again.")
                        except Exception as e:
                            logger.error(f"Command processing failed: {e}")
                    input_buffer = ""
                else:
                    input_buffer += char
        else:
            if select.select([sys.stdin], [], [], 0.1)[0]:
                char = sys.stdin.read(1)
                if char in ['\n', '\r']:
                    action = input_buffer.strip().lower()
                    if action == "cmd":
                        try:
                            command = input("Enter command ('OTA' or 'reverse old firmware'): ").strip().lower()
                            if command == "ota":
                                file_path = input("Enter firmware file path: ").strip()
                                if os.path.exists(file_path):
                                    send_ota_firmware(file_path)
                                else:
                                    logger.error(f"File not found: {file_path}")
                            elif command == "reverse old firmware":
                                client.publish(OTA_TOPIC, "reverse old firmware", qos=1)
                                logger.info("Sent: reverse old firmware")
                            else:
                                logger.warning("Invalid command")
                        except EOFError:
                            logger.error("Input interrupted (EOF). Please try again.")
                        except Exception as e:
                            logger.error(f"Command processing failed: {e}")
                    input_buffer = ""
                else:
                    input_buffer += char
        time.sleep(0.01)

def setup_mqtt():
    global client
    client = mqtt.Client(
        client_id="Python_Server_" + str(int(time.time())),
        protocol=mqtt.MQTTv311,
        callback_api_version=mqtt.CallbackAPIVersion.VERSION2
    )
    client.username_pw_set(USERNAME, PASSWORD)
    client.tls_set(ca_certs=CA_CERT)  # Ensure CA_CERT path is correct
    client.on_connect = on_connect
    client.on_message = on_message

    try:
        client.connect(BROKER, PORT)
        logger.info("Connecting to MQTT broker...")
    except Exception as e:
        logger.error(f"Failed to connect: {e}")
        sys.exit(1)

    mqtt_thread = threading.Thread(target=client.loop_forever, daemon=True)
    mqtt_thread.start()
    input_thread = threading.Thread(target=input_handler, daemon=True)
    input_thread.start()

    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        logger.info("Received KeyboardInterrupt, shutting down...")
        client.loop_stop()
        client.disconnect()
        time.sleep(0.1)
        logger.info("Shutdown complete")
        sys.exit(0)

if __name__ == "__main__":
    logger.info(f"Server Public Key:\n{PUBLIC_KEY_PEM}")
    setup_mqtt()