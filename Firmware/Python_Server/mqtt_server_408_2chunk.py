...
def sign_firmware(file_path):
    ...
    r, s = ec._decode_dss_signature(signature)
...
def send_ota_firmware(file_path, device_id=None):
    ...
    with open(file_path, "rb") as f:
        ...
        while chunk_num < total_chunks and ota_in_progress:
            chunk = f.read(min(CHUNK_SIZE, file_size - total_bytes_sent))
        ...
        if ota_in_progress:
            while ota_in_progress and len(acknowledged_chunks) < total_chunks:
                ...
                if not chunk_ack_event.wait(timeout=FINAL_CONFIRMATION_TIMEOUT):
                    ...
                    if missing_chunks:
                        client.publish(OTA_TOPIC, f"OTA:REQUEST:{','.join(map(str, missing_chunks))}", qos=1)
            if ota_in_progress:
