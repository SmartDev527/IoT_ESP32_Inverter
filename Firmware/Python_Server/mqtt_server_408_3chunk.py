...
def handle_provisioning(uuid, nonce, pubkey_b64):
    ...
    try:
        ...
        creds = f"DEVICE_ID:{device_id}:USERNAME:ESP32_{device_id}:PASSWORD:{password}"
    ...
    try:
        ...
        with open(file_path, "rb") as f:
            ...
            while total_bytes_sent < file_size and ota_in_progress:
                ...
                if not chunk:
                    print(f"Unexpected empty read at chunk {chunk_num}, total_bytes_sent: {total_bytes_sent}")
                    break
                ...
                for attempt in range(MAX_RETRIES):
                    ...
                    if chunk_ack_event.wait(timeout=20):
                        ...
                        print(f"Chunk {chunk_num} acknowledged - OTA Progress: {progress_percentage:.2f}% ({total_bytes_sent}/{file_size} bytes)")
                        break
                    elif attempt == MAX_RETRIES - 1:
                        ...
                        ota_in_progress = False
                        return
                    print(f"Retrying chunk {chunk_num} after {RETRY_DELAY} seconds")
                    time.sleep(RETRY_DELAY)
            ...
            if ota_in_progress:
                ...
                progress_percentage = (total_bytes_sent / file_size) * 100
                print(f"OTA completed successfully - Final Progress: {progress_percentage:.2f}%")
    except Exception as e:
        ...
        if platform.system() == "Windows":
            if msvcrt.kbhit():
                ...
                if char in ['\r', '\n']:
                    ...
                    if action == "cmd":
                        command = input("Enter command ('ota', 'reset soft', 'reset factory', 'reverse'): ").strip().lower()
                        device_id = input("Enter IMEI (or press Enter for all devices, but device-specific is recommended): ").strip() or None
                        if command == "ota":
                            ...
                            msg = "RESET:SOFT" + (f":DEVICE:{device_id}" if device_id else "")
                            client.publish(COMMAND_TOPIC, msg, qos=1)
                            print(f"Sent: {msg}")
                        elif command == "reset factory":
                            if not device_id:
                                print("Warning: Factory reset requires a device ID to prevent unintended resets. Please specify an IMEI.")
                                device_id = input("Enter IMEI: ").strip()
                            msg = f"RESET:FACTORY:DEVICE:{device_id}"
                            client.publish(COMMAND_TOPIC, msg, qos=1)
                            print(f"Sent: {msg}")
                        elif command == "reverse":
                            msg = "reverse old firmware" + (f":DEVICE:{device_id}" if device_id else "")
            ...
            if select.select([sys.stdin], [], [], 0.1)[0]:
                ...
                if char in ['\n', '\r']:
                    ...
                    if action == "cmd":
                        command = input("Enter command ('ota', 'reset soft', 'reset factory', 'reverse'): ").strip().lower()
                        device_id = input("Enter IMEI (or press Enter for all devices): ").strip() or None
                        if command == "ota":
                            file_path = input("Enter firmware file path: ").strip()
                            if os.path.exists(file_path):
                                send_ota_firmware(file_path, device_id)
                            ...
                            msg = "RESET:FACTORY" + (f":DEVICE:{device_id}" if device_id else "")
                            client.publish(COMMAND_TOPIC, msg, qos=1)
                            print(f"Sent: {msg}")
                        elif command == "reverse":
                            msg = "reverse old firmware" + (f":DEVICE:{device_id}" if device_id else "")
