...
enum SetupState {
  ...
  STATE_RUNNING,
  STATE_ERROR,
  STATE_STOPPED  // Added from latest
};
...
const uint32_t WDT_TIMEOUT = 30;  // Added from latest
...
String clientID = "ESP32_SIM7600_" + String(millis());  // Reverted to working code’s static ID
String imei = "";  // Kept for response enhancement
...
void loop() {
  esp_task_wdt_reset();  // Added from latest
...
void handleMessage(String topic, String payload) {
  ...
  if (payload == "STOP_DEVICE") {  // Added from latest
  ...
  String reply = imei + ": " + payload;  // Enhanced with IMEI from latest
