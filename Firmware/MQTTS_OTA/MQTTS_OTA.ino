...
enum SetupState
{
  ...
  STATE_ERROR,
  STATE_STOPPED,
  STATE_RECOVER_NETWORK, // Added
  STATE_RECOVER_GPRS,    // Added
  STATE_RECOVER_MQTT     // Added
};
...
const char *mqtt_topic_firmware = "firmware/update"; // New OTA topic
...
unsigned long lastChunkNum = 0;
unsigned long chunkCount = 0; // Add to global variables
unsigned long lastReportedSize = 0;
std::vector<unsigned long> missingChunks;
std::map<unsigned long, bool> receivedChunks; // Track received chunk numbers
...
void setup()
{
  ...
  missingChunks.clear();
  Update.abort(); // Ensure any partial OTA is terminated
...
void loop()
{
  ...
  case STATE_ERROR:
    ...
    delay(2000);
    ESP.restart(); // Force restart instead of infinite loop
    break;
...
String encryptMessage(const char *message)
{
  ...
  // Use dynamic allocation with bounds checking
...
void resetState()
{
  ...
  // Cleanup OTA resources
  if (otaInProgress)
  {
    cleanupResources(); // Use the cleanup function for consistency
  }
...
bool subscribeMQTT()
{
  // Subscribe to server_cmd (for existing non-OTA logic)
  ...
  // Subscribe to firmware/update (for OTA)
...
void startOTA(uint32_t totalSize)
{
  ...
  {
    SerialMon.println("OTA begin failed: " + String(Update.getError()));
...
// Process raw binary firmware data
void processOTAFirmware(const String& topic, byte* payload, unsigned int dataLen) {
    ...
    SerialMon.println("Encoded payload: " + encodedPayload.substring(0, min(64U, encodedPayload.length())) + (encodedPayload.length() > 64 ? "..." : ""));

    // Decode Base64
    size_t maxDecodedLen = (encodedPayload.length() / 4) * 3;
    unsigned char decodedPayload[maxDecodedLen];
    size_t decodedLen = base64_decode(encodedPayload.c_str(), decodedPayload, maxDecodedLen);
    if (decodedLen == 0) {
        SerialMon.println("Base64 decode failed");
        return;
    }
    SerialMon.println("Decoded length: " + String(decodedLen));
    SerialMon.print("Decoded payload (first 16 bytes): ");
    for (size_t i = 0; i < min(16U, decodedLen); i++) {
        if (decodedPayload[i] < 0x10) SerialMon.print("0");
        SerialMon.print(decodedPayload[i], HEX);
        SerialMon.print(" ");
    }
    SerialMon.println("...");
    ...
    if (decodedStr.startsWith("OTA:BEGIN:")) {
        if (!otaInProgress) {
            ...
            SerialMon.println("Starting OTA with size: " + String(totalSize));
            startOTA(totalSize);
        } else {
            SerialMon.println("OTA already in progress, ignoring BEGIN");
            cleanupResources();
            otaInProgress = false;
            SerialMon.println("Resetting OTA state due to unexpected BEGIN");
            uint32_t totalSize = decodedStr.substring(10).toInt();
            SerialMon.println("Starting OTA with size: " + String(totalSize));
            startOTA(totalSize);
        }
    ...
    if (decodedStr == "OTA:END") {
        if (otaInProgress) {
            ...
            SerialMon.println("OTA not in progress, ignoring END");
        }
        return;
    }
    ...
    if (Update.write(decodedPayload + 4, chunkSize) != chunkSize) {
        SerialMon.println("Write failed for chunk " + String(chunkNum));
        cleanupResources();
        otaInProgress = false;
...
void finishOTA()
{
  ...
  if (otaReceivedSize != otaTotalSize)
  {
    ...
    cleanupResources(); // Clear OTA data on failure
  ...
  if (!Update.end())
  {
    SerialMon.println("OTA end failed: " + String(Update.getError()));
    cleanupResources(); // Clear OTA data on failure
  ...
  // Clear OTA state before restart
  otaInProgress = false;
  otaReceivedSize = 0;
  otaTotalSize = 0;
  chunkCount = 0;
...
{
  missingChunks.clear();
  unsigned long expectedChunks = otaReceivedSize / 512; // Assuming 512-byte chunks
